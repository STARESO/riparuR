#' ---
#' title : "RIPARU - microplastiques time series"
...
#' =============================================================================
#'
#' MARE VIVU RIPARU - MICROPLASTIQUES TIME SERIES
#'
#' Description :
...
microplastics_time_total <- microplastics %>%
     ...
     summarise(
          Moyenne_normalise = mean(Total_normalise, na.rm = FALSE),
          N_sites = n()
     ) %>%
...
# Types separated
g1 <- ggplot(microplastics_time_total, aes(
     x = Annee_Saison, y = log(1 + Moyenne_normalise), group = Type,
     color = Type
)) +
     geom_point() +
     ...
     labs(
          title = "Microplastiques - Moyenne tous sites",
          x = "",
...
# Types separated
g3 <- ggplot(microplastics_time_total, aes(
     x = Annee_Saison, y = Moyenne_normalise, group = Type,
     color = Type
)) +
     geom_point() +
...
g4 <- ggplot(microplastics_time_total, aes(
     x = Annee_Saison, y = Moyenne_normalise, group = Type,
     color = Type
)) +
     geom_line() +
